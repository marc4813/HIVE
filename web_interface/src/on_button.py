...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if not button.started_sim and button.start_stack_cmd and not button.started_stack:
            fleet_launch.start()
        ...
        if button.started_sim and button.stop_sim_cmd:
            ...
            stack_uuid= roslaunch.rlutil.get_or_generate_uuid(None, False)
            roslaunch.configure_logging(stack_uuid)
            sim_launch = roslaunch.parent.ROSLaunchParent(stack_uuid, [rp.get_path('fleet_bringup') + '/launch/bringup.launch'])
